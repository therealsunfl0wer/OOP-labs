...
protected:
  ...
  void DrawPreview(QPainter &painter) const override {
    ...
    painter.drawRect(QRect(QPoint(xstart - std::abs(xend - xstart),
                                  ystart - std::abs(yend - ystart)),
                           QPoint(xstart + std::abs(xend - xstart),
                                  ystart + std::abs(yend - ystart)))
                         .normalized());
  }
};
...
protected:
  ...
  void DrawPreview(QPainter &painter) const override {
    QRect rect(QPoint(xend, yend), QPoint(xstart, ystart));
    painter.setPen(QPen(Qt::black, 1, Qt::DashLine));
    painter.setBrush(Qt::NoBrush);
